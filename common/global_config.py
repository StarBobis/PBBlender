...
class LogicName:
    ...
    def is_zzmi_family(cls, logic_name: str) -> bool:
        return logic_name in {cls.ZZMI, cls.ZZZ}
...
class GlobalConfig:
    ...
    def read_from_main_json_ssmt4(cls) :
        try:
            # ProjectBunny is preferred when present; otherwise fall back to
            # the SSMT4 settings so the add-on still works with older tools.
            main_settings = GlobalConfig._projectbunny_settings() or GlobalConfig._ssmt_settings()
            cls.gamename = main_settings.get("CurrentGameName", "")
            # ProjectBunny / SSMT records the workspace name per game in
            # CurrentWorkSpaceByGame, which has higher priority than the global
            # CurrentWorkSpace.
            ...
            # ProjectBunny writes its cache folder path under DBMTWorkFolder,
            # same key used by SSMT4. When empty, fall back to the default.
            ...
            # Only trust the game config written by the same tool that wrote
            # settings.json, so MMT / MIMITools configs do not leak in.
            game_config_json_path = ""
            config_root = (
                GlobalConfig.path_projectbunny_global_configs_folder()
                if GlobalConfig._projectbunny_settings()
                else GlobalConfig.path_ssmt4_global_configs_folder()
            )
            ...
            # ProjectBunny can also store these fields directly in settings.json.
            if not cls.current_game_migoto_folder:
                cls.current_game_migoto_folder = str(main_settings.get("MigotoFolder", "") or "").strip()
            if not cls.logic_name:
                cls.logic_name = str(main_settings.get("GamePreset", "") or "").strip()
    ...
    @staticmethod
    def _projectbunny_settings():
        # Read ProjectBunny's own settings file only.
        return GlobalConfig._load_json_dict(
            os.path.join(GlobalConfig.path_projectbunny_global_configs_folder(), "settings.json")
        )
    ...
    @staticmethod
    def path_projectbunny_global_configs_folder():
    ...
    def path_main_json_ssmt4():
        for folder, filename in (
            (GlobalConfig.path_projectbunny_global_configs_folder(), "settings.json"),
